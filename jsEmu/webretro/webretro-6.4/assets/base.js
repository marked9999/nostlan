...
var pdKeys = [
	8, 9, 13, 19, 27, 32, 33, 34, 35, 36, 42, 44, 45, 91, 92, 93, 112, 113, 114, 115, 116, 117, 118, 119, 120, 121, 122,
...
var smasBrickFix = {
	'16a160ddd431a3db6fcd7453ffae9c4c': [
		80, 65, 84, 67, 72, 0, 127, 160, 0, 8, 169, 1, 133, 160, 141, 0, 22, 107, 1, 191, 182, 0, 4, 34, 160, 255, 0, 6,
		189, 164, 0, 4, 34, 160, 255, 0, 69, 79, 70
	],
	e87d43969bdf563d1148e3b35e8b5360: [
		80, 65, 84, 67, 72, 0, 129, 160, 0, 8, 169, 1, 133, 160, 141, 0, 22, 107, 1, 193, 182, 0, 4, 34, 160, 255, 0, 6,
		191, 164, 0, 4, 34, 160, 255, 0, 69, 79, 70
	],
	'2071b049a463cefd7a0b7aeab8037ca0': [
		80, 65, 84, 67, 72, 0, 127, 160, 0, 8, 169, 1, 133, 160, 141, 0, 22, 107, 1, 191, 190, 0, 4, 34, 160, 255, 0, 6,
		189, 164, 0, 4, 34, 160, 255, 0, 69, 79, 70
	]
...
var savedKeybindsObj = window.localStorage.getItem('RetroArch_settings_keybinds')
...
function createConfigList() {
	...
	for (var i = 0; i < validKeybinds.length; i++) {
		keybindTable.innerHTML +=
			'<tr><td>' + validKeybinds[i] + '</td><td>' + keybindsObj[validKeybinds[i]] + '</td></tr>';
	}
...
function createScreenshotList() {
	var screenshots = FS.analyzePath('/home/web_user/retroarch/userdata/screenshots/').exists
		? FS.readdir('/home/web_user/retroarch/userdata/screenshots/').filter((k) => !['.', '..'].includes(k))
		: [];
	...
	if (e.target.tagName == 'SPAN') {
		...
		switch (e.target.dataset.action) {
			...
			case 'delete':
				if (
					confirm(
						'Are you sure you want to delete this ' + saveIDs[saveNo].type + ' for "' + saveIDs[saveNo].name + '"?'
					) &&
...
function autodetectCore(name, data) {
	...
	if (allValidFileExts.split(', ').includes(nameExt)) {
		...
		alert(
			'That is a ' + detectedSystem + ' file! ' + detectedCore + ' (' + detectedSystem + ') is not currently supported.'
		);
	}
}
		...
		document.addEventListener(
			...
			function (e) {
				if (
					!stateReaders2Ready &&
					e.code ==
						Object.keys(codeToConfigIDMap).find((k) => codeToConfigIDMap[k] == savedKeybindsObj.input_load_state)
				)
					readyStateReaders2();
			},
			false
...
function afterStart() {
	...
	document.addEventListener(
		...
		function (e) {
			if (
				!stateReadersReady &&
				e.code == Object.keys(codeToConfigIDMap).find((k) => codeToConfigIDMap[k] == savedKeybindsObj.input_save_state)
			)
				readyStateReaders();
		},
		false
